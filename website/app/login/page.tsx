...
import { Mail, Loader2, User, BookOpen, Building, ArrowLeft, Smartphone, KeyRound } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const handleRequestOtp = async (e: React.FormEvent) => {
    ...
    try {
      const endpoint = view === 'partner' ? AuthEndpoints.requestPartnerOtp : AuthEndpoints.requestLoginOtp;
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      const { data } = await api.post(AuthEndpoints.verifyOtp, { email, token: otp });
      setAuth(data.user);
  ...
  const handleResendOtp = async () => {
    ...
    setError('');
    try {
      await api.post(AuthEndpoints.resendOtp, { email });
  ...
  const handleGoogleSuccess = async (credentialResponse: any) => {
    ...
    setError('');
    try {
      const { data } = await api.post(AuthEndpoints.googleLogin, {
      ...
      <div>
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-brand-blue/20 focus:border-brand-blue transition-all text-slate-900 font-medium tracking-[0.5em] text-center"
            placeholder="000000"
          />
        </div>
        <div className="mt-2 text-right">
          <button
            ...
            disabled={loading}
            className="text-sm text-[#0077CB] font-semibold hover:underline disabled:opacity-50"
          >
            Resend OTP
          </button>
        </div>
    ...
    <div className="flex items-center justify-center min-h-[calc(100vh-64px)] px-6 py-12">
      {view === 'selection' && (
        <div className="bg-white p-8 sm:p-12 rounded-[32px] border border-slate-200 shadow-xl shadow-blue-500/5 max-w-md w-full">
          ...
          <div className="space-y-4">
            <button 
              onClick={() => { setView('student'); resetForm(); }}
            ...
            <button 
              onClick={() => { setView('download'); resetForm(); }}
              className="w-full flex items-center p-4 border border-slate-200 rounded-2xl hover:border-[#0077CB] hover:bg-blue-50/50 transition-all group text-left"
            >
            ...
            <button 
              onClick={() => { setView('partner'); resetForm(); }}
      ...
      {view === 'student' && (
        <div className="bg-white p-8 sm:p-12 rounded-[32px] border border-slate-200 shadow-xl shadow-blue-500/5 max-w-md w-full animate-in fade-in slide-in-from-bottom-4 duration-300">
          <button onClick={() => { setView('selection'); resetForm(); }} className="flex items-center text-slate-500 hover:text-slate-800 transition-colors mb-6 text-sm font-medium">
            <ArrowLeft size={16} className="mr-1" /> Back
          </button>
