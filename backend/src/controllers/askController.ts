...
export const createQuestion = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (config.maxQuestionWords && getWordCount(text) > config.maxQuestionWords) {
      return res.status(400).json({
        error: `Question exceeds the maximum word limit of ${config.maxQuestionWords} words.`,
      });
    }
...
export const updateQuestion = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Validate config word limits if config is defined
