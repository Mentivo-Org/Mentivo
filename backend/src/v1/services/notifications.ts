...
export async function sendCallWarningNotification(studentId: string) {
  try {
    const prisma = (await import('../config/db.ts')).default;
    await prisma.notification.create({
      data: {
        userId: studentId,
        title: 'Call ending soon ⚠️',
        body: 'You have 3 minutes of balance left. Top up or the call will end.',
      },
    });
    ...
    console.error('Failed to send call warning push notification:', error);
  }
}

export async function sendScheduledCallReminder(userId: string, opponentName: string, isMentor: boolean) {
  try {
    const prisma = (await import('../config/db.ts')).default;
    const title = 'Upcoming Call 📅';
    const body = `You have a scheduled call with ${opponentName} in 10 minutes!`;
    ...
    console.error('Failed to send scheduled call reminder push notification:', error);
  }
}
