...
export const recalculateTopMentors = async () => {
    try {
        console.log('[TopMentors v2] Recalculating top mentors cache...');
        ...
        console.log(`[TopMentors v2] Successfully recalculated and cached ${formatted.length} top mentors.`);
        return formatted;
    } catch (err) {
        console.error('[TopMentors v2] Error recalculating top mentors:', err);
        throw err;
    }
};
