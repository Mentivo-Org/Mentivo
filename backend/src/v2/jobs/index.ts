...
async function notifyUpcomingCalls() {
    try {
        const now = new Date();
        const tenMinsLaterStart = new Date(now.getTime() + 9 * 60 * 1000);
        const tenMinsLaterEnd = new Date(now.getTime() + 11 * 60 * 1000);

        const upcomingCalls = await prisma.callSession.findMany({
            where: {
                status: 'scheduled',
                scheduledAt: {
                    gte: tenMinsLaterStart,
                    lte: tenMinsLaterEnd
                }
            },
            include: {
        ...
        const { sendScheduledCallReminder } = await import('../services/notifications.ts');

        for (const call of upcomingCalls) {
            const reminderKey = `reminder-sent:scheduled_call:${call.id}`;
            const alreadySent = await redis.get(reminderKey);
            if (!alreadySent) {
                await redis.set(reminderKey, 'true', 'EX', 3600); // 1 hour expiration
                
                // Send reminder to student
                const mentorName = call.mentor.name || 'Mentor';
                await sendScheduledCallReminder(call.student_id, mentorName, false);

                // Send reminder to mentor
                const studentName = call.student.name || 'Student';
                await sendScheduledCallReminder(call.mentor_id, studentName, true);
                
                console.log(`[Reminder] Sent 10-minute upcoming call reminder for call ${call.id}`);
            }
        }
    } catch (err) {
        console.error('[Reminder] Error in notifyUpcomingCalls job:', err);
    }
}
